      ...
      *| DATE GENERATED: 08/02/2022 01:07                              |
      *| ID: ec8d0887-6bf9-4e22-bd47-13947cbeb1a7                      |
      *+---------------------------------------------------------------+
      *+---------------------------------------------------------------+
      *| TEST_TEST3                                                    |
      ...
      *+---------------------------------------------------------------+
       ...
       01 PROGRAM-NAME   PIC X(8)  VALUE 'CBSCSDG'.
       01 BZ-ASSERT.
      ...
      *+---------------------------------------------------------------+
       ...
       1 AZ-TEST-EXPECTED-DATA-VALUE.
          ...
          3 ZUT00000001.
            5 PIC X(10) DISPLAY VALUE '2022-08-02'.
          3 ZUT00000002.
            5 PIC X(8) DISPLAY VALUE '01.06.41'.
          3 ZUT00000003.
            5 PIC X(20) DISPLAY VALUE 'CUSTOMER DEREGISTERE'.
            5 PIC X(13) DISPLAY VALUE 'D SUCESSFULLY'.
            5 PIC X(67) DISPLAY VALUE SPACES.
       01 AZ-COMPARE.
         03 AZ-COMPARE-ITEM-NAME-PTR  POINTER.
         03 AZ-COMPARE-ITEM-NAME-LEN  PIC S9(9) COMP-5.
         03 AZ-COMPARE-ITEM-VALUE-PTR POINTER.
       ...
       1 AZ-COMPARE-WORK-ITEMS.
          ...
          3 ZUT0000000D PIC X(100) OCCURS 2.
       LINKAGE SECTION.
       01 AZ-TEST                   PIC X(80).
       01 AZ-ARG-LIST.
         03 ARG-LENGTH PIC 9(4) COMP-4.
      ...
      * EVALUATE OUTPUT VALUE
           MOVE 0 TO RETURN-CODE
           IF CUSTOMER-NAME OF CSRGRES OF CSDGRES OF DFHCOMMAREA =
           ZUT00000000 THEN
             CONTINUE
           ELSE
             MOVE CUSTOMER-NAME OF CSRGRES OF CSDGRES OF DFHCOMMAREA
           TO ZUT00000005(1)
           ...
           END-IF
           IF (CUSTOMER-ID OF CSRGRES OF CSDGRES OF DFHCOMMAREA IS
           NUMERIC)
               AND (CUSTOMER-ID OF CSRGRES OF CSDGRES OF DFHCOMMAREA =
           78100) THEN
             CONTINUE
           ELSE
             MOVE CUSTOMER-ID OF CSRGRES OF CSDGRES OF DFHCOMMAREA TO
           ZUT00000007(1)
           ...
           ZUT00000007(2)
             ...
             PERFORM THROW-ASSERTION
           END-IF
           IF SYS-DATE OF CSRGRES OF CSDGRES OF DFHCOMMAREA =
           ZUT00000001 THEN
             CONTINUE
           ELSE
             MOVE SYS-DATE OF CSRGRES OF CSDGRES OF DFHCOMMAREA TO
           ZUT00000009(1)
           ...
           ZUT00000009(2)
             ...
             PERFORM THROW-ASSERTION
           END-IF
           IF SYS-TIME OF CSRGRES OF CSDGRES OF DFHCOMMAREA =
           ZUT00000002 THEN
             CONTINUE
           ELSE
             MOVE SYS-TIME OF CSRGRES OF CSDGRES OF DFHCOMMAREA TO
           ZUT0000000B(1)
           ...
           ZUT0000000B(2)
             ...
             PERFORM THROW-ASSERTION
           END-IF
           IF MESSAGES OF CSRGRES OF CSDGRES OF DFHCOMMAREA =
           ZUT00000003 THEN
             CONTINUE
           ELSE
             MOVE MESSAGES OF CSRGRES OF CSDGRES OF DFHCOMMAREA TO
           ZUT0000000D(1)
             MOVE ZUT00000003 TO ZUT0000000D(2)
      ...
      *+---------------------------------------------------------------+
       ...
       PROCEDURE DIVISION.
      * SET INPUT VALUE
           ENTRY "PGM_INPT_CBSCSDG" USING AZ-TEST AZ-INFO-BLOCK
           DFHEIBLK DFHCOMMAREA.
           DISPLAY 'PGM_INPT_CBSCSDG CHECK VALUES...'.
           MOVE 0 TO RETURN-CODE.
           ...
           PERFORM TEARDOWN.
      * EVALUATE OUTPUT VALUE
           ENTRY "PGM_OUTP_CBSCSDG" USING AZ-TEST AZ-INFO-BLOCK
           DFHEIBLK DFHCOMMAREA.
           DISPLAY 'PGM_OUTP_CBSCSDG INPUT VALUES...'.
           MOVE 4 TO RETURN-CODE.
           INSPECT AZ-TEST TALLYING AZ-TEST-NAME-LEN FOR CHARACTERS
             BEFORE INITIAL SPACE.
           EVALUATE AZ-TEST(1:AZ-TEST-NAME-LEN)
      ...
      *+---------------------------------------------------------------+
       ...
       01 AZ-TESTCASE-ID        PIC X(36)
           VALUE 'ec8d0887-6bf9-4e22-bd47-13947cbeb1a7'.
       LINKAGE SECTION.
       01 AZ-TEST               PIC X(80).
      ...
      *+---------------------------------------------------------------+
       ...
       PROCEDURE DIVISION.
      * CHECK OUTPUT VALUE
      * CICS_INPT_0E08_CBSCSDG.
           ENTRY 'CICS_INPT_0E08_CBSCSDG' USING AZ-TEST
           AZ-INFO-BLOCK AZ-DFHEIBLK AZ-DFHCOMMAREA ARG0 ARG1 ARG2
           ARG3 ARG4 ARG5 ARG6 ARG7 ARG8 ARG9 ARG10.
           DISPLAY 'CICS_0E08_CBSCSDG CHECK VALUES...'
           MOVE 4 TO RETURN-CODE.
      ...
      * EXEC CICS RETURN X'0000'
           ...
           PERFORM TEARDOWN.
      * SET INPUT VALUE
      * CICS_OUTP_0E08_CBSCSDG.
           ENTRY 'CICS_OUTP_0E08_CBSCSDG' USING AZ-TEST
           AZ-INFO-BLOCK AZ-DFHEIBLK AZ-DFHCOMMAREA ARG0 ARG1 ARG2
           ARG3 ARG4 ARG5 ARG6 ARG7 ARG8 ARG9 ARG10.
           DISPLAY 'CICS_0E08_CBSCSDG INPUT VALUES...'
           MOVE 0 TO RETURN-CODE.
           MOVE 0 TO AZ-TEST-LEN.
           INSPECT AZ-TEST TALLYING AZ-TEST-LEN FOR
           CHARACTERS BEFORE INITIAL SPACE.
      ...
      * EXEC CICS RETURN X'0000'
       ...
       GETLINENUM.
           ...
           EXIT.
       TEARDOWN.
           DISPLAY 'CICS_0E08_CBSCSDG SUCCESSFUL.'
           GOBACK.
       END PROGRAM 'CICS_0E08_CBSCSDG'.
